# Spec Scraper v2 — Полная документация (текущее состояние)

## 1. Что это и зачем нужно

`spec_scraper` — это ETL/ingest-пайплайн для наполнения RAG-корпуса документами по электротехнике и смежной нормативке.
